...
void main() {
  // A "single character" slot is a String almost everywhere, so nothing stops none or 20.
  // #region ascii
  ...
  // One character to a reader is one grapheme cluster. The skin tone, flag and family below are 2,
  // 2 and 5 code points, each filling one slot.
  // #region char
  ...
  // The plurals hold text, so they're extension types over String, with the elements on a getter.
  // #region plurals
  ...
  // A range over a number with no standard text form, so `tryFrom(int)` and no `parse`. Zero is the
  // one value these 2 disagree on.
  // #region quantities
  ...
  // Each width is its own type, so a Uint8 can't be passed where a Uint4 is wanted.
  // #region fixedwidths
  print(Uint8.tryFrom(255)?.value); // 255
  print(Uint8.tryFrom(256)); // null (refused, not truncated to 0)
  // #endregion

  // The other kind: bounds nothing, names the unit, since 15 and 0.15 are both plausible readings of
  // "15 percent".
  // #region percentage
  ...
  // The bounded one. Its range states the convention, so one door is enough.
  // #region probability
