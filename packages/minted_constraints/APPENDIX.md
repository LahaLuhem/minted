...
the getters returned `String` and the parameters took `Digits`: the 2 halves of one type disagreed
...
nothing per value, and `.value` is the number. Arity decides how each consumer exposes its digits.
`Iban.checkDigits` is always exactly 2, so it is a `({Digit first, Digit second})` record (a
...
roughly 8 times the bytes of the string it came from. So `Digits` is backed by a `Uint8List`:
one byte per digit, and a real `Uint8Array` on the web (where a `String` would be 2-byte UTF-16).
...
`Digits` is an `@immutable` class, not an extension type, for 2 reasons. Value equality: an
...
at 2 digits per byte, or tighter) can replace it behind the same `Iterable<Digit>` / `operator []`
/ `asString` interface without touching callers. Packing is deferred, since it only pays off at
...
**`Char` is one grapheme cluster, not one code unit or code point.** A skin-toned thumbs-up is 2
code points, a flag 2, a joined family 5, and each fills one slot, so `length == 1` refuses all 3.
It also *admits* half a surrogate pair, which is why `Char` carries an explicit unpaired-surrogate
guard: `characters` counts a lone surrogate as one grapheme too. Control characters and CRLF are
single graphemes and stay admitted, since excluding them would invent a rule the concept lacks.

**2 families of letters, because one type cannot do both jobs.** A Danish initial is `Ø` and a Polish
one `Ł`, so `AsciiLetter` cannot serve human text, and `[A-Za-z]` is exactly what ISO 9362 and the IBAN
...
reversible: measured, `'a' + ZWJ + 'b'` stays 2 graphemes, UAX #29 joining across ZWJ only for
emoji. Text should print and compare as text, where `Digits` should not.
...
range, where [`Percentage`](#percentage-constraint-type) constrains the unit instead. *Primitive*, not
...
**2 exceptions, both where an inherited member would read as a lie.** `Percentage` holds `15` for
15 percent, so `percentage * 200` would compute `3000` while reading as 15 percent of 200.
`Char`, `Letter` and `Letters` guarantee characters where `length` counts code units, which for one
emoji is `2`. The 5 `Ascii*` types escape it, code units and characters coinciding there, and
...
in a nibble slot and the analyzer stays silent. As separate types it rejects that, which is the point.
...
`Uint16` says how many bits a value fits in, not what it means, so 2 unrelated domains of the same
...
the right shape and even the same `static Uint8? tryFrom(int)`, but that is one door out of 7:
...
are always disabled and cannot be enabled". Re-exporting it would ship 4 unvalidated public
...
canonical form, so the stored unit is the one that has to render cleanly, and the 2 conventions
are not symmetric:
...
the shortest round-tripping decimal 2 places over via `toStringAsExponential`, so `0.29` lands on
`29`. The non-finite guard runs first, since a `NaN` has no exponent to bump.

**2 doors, where every other constraint type has one.** The bare door is the percent because the
name states the unit, and the fraction door is spelled out so a caller holding a ratio cannot get it
...
**`double`, not a fixed-point `int`.** Fixed point was the tempting answer to `0.1 + 0.2`, and 3
...
`Percentage` would split one pair across 2 files.
...
`0.29`, about a 3rd of ordinary values. Nothing fixes that short of a representation the package
