...
part '../constants/probability_constants.dart';

/// A probability: `0` to `1` inclusive, where `0.15` is a 15 percent chance.
///
/// The range states the convention, so unlike [Percentage] there is no unit to get wrong and one door
/// is enough.
///
/// > [!NOTE]
...
///
/// Named values: [ProbabilityConstants].
///
...
extension type const Probability._(double value) implements double {
  /// The [Probability] with numeric [value], or `null` unless it's `0` to `1`. A `NaN` is out of range,
  /// the bound being written as a positive test.
  static Probability? tryFrom(num value) =>
      value >= _impossible && value <= _certain ? ._(positiveZeroed(value.toDouble())) : null;

  /// The [Probability] equal to [percentage], or `null` if it sits outside `0`-`100`. Partial where
  /// [toPercentage] is total, because a [Percentage] is unbounded.
  ...
  /// The probability of this *not* happening. Always in range, so it never fails, but not exactly involutive:
  /// doubles make `0.3`'s double complement `0.30000000000000004`. Why: `APPENDIX.md#probability-constraint-type`.
