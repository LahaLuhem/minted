...
Zurich and to Sydney next to Sydney. 2 equal strings, 2 continents, so sharing a type would have
`==` say yes across an ocean. `ShortPlusCode` is its own type, `recoverNear` is the only way across,
and there is no `bounds` or `centre` on it because there is nothing to return. Same split as
`Hostname` and `DnsName`, with a reference coordinate as the extra input.

**The engine is trusted for its maths and its `isValid`, and for nothing else.** `isFull` and
`isShort` skip the validity check and answer a narrower question about shape, and `decode` guards on
`isFull`, so an invalid code decodes instead of being refused: `8FWC2_45+G6`, underscore and all,
comes back as a real spot in Germany. Against Google's own Python over 336k generated codes:
...
So both doors ask `isValid` and then read full versus short off the separator's position themselves.

**Out-of-range input cannot reach the engine, which is the rest of why wrapping is safe.** The
encoding vectors that fail against this package all hand it a latitude or longitude outside the
legal range, which it does not normalise. Every door here takes a `GeoCoordinate`, so not one of
those inputs can be spelled.

**Cell edges land within a micrometre of the standard's vectors, not on them.** `decoding.csv` writes
its edges to a fixed number of decimals, so exact comparison fails on most rows here and on Google's
own Python too. Worst gap across the file is about 5 micrometres, showing up as `bounds.bbox`
...
pass them around, where nobody holds a code length.
