...
dependency-free here (validation is a regex plus 2 nibble reads). Same shape as
[`Date`][date-value-type] filling the date-only gap: minted adds the missing *value* rather than
re-implementing the neighbouring tool. (Both are named `Uuid`, so a consumer using both imports one
with a prefix.)
...
**Accept every well-formed UUID, classify rather than reject.** [Hard rule 3][hard-rules] wants the
real standard rather than a regex shape, but a UUID carries no checksum, and RFC 9562 treats its version
...
so its weights are powers of 2 mod 11 where `mod11_check_character.dart` descends linearly. They
share a modulus and an `X` glyph and agree on nothing: the weighted one rejected all 5 ORCID iDs
tested against it. Hence a separate file, and a test pinning that the 2 cannot be swapped, because
that resemblance is exactly what invites a "simplification".

**The block test compares text, not integers.** 16 digits reach 10^16, past the web's safe
...
## Isbn: 2 generations, one value, no hyphens

ISO 2108 has had 2 shapes: 10 characters with a mod-11 check digit (10 spelled `X`), and, since
2007, 13 digits with the GS1 mod-10 check. Every 978-prefixed ISBN-13 has exactly one ISBN-10
twin, so the same book arrives written both ways.

**Everything folds to 13 digits.** Extension-type equality is representation equality, so the
...
across, because the 2 generations use different algorithms.
...
precedent: grouping by 4 needs no table. The `isbn` package on pub carries no range data either,
so it would buy only the 2 check-digit algorithms, which is the case [hard rule 7][hard-rules]
...
the 10-digit form catches every transposition. See [check-digit blind
...
inconsistent and is not, because the 2 hyphens are different things. An ISBN's groups come from
ISBN International's range table, so rendering them [ships a clock][registry-data-ships-a-clock]. An
ISSN's single hyphen sits after the 4th character always, fixed by ISO 3297 and derivable from
...
**`checkCharacter` is a `String`, not a `Digit`.** ISO 3297 spells the value 10 as `X`, so the
...
without a [caveat to pin][check-digit-blind-spots]. The tests pin the opposite: 3 real ISSNs
with an adjacent pair differing by 5, transposed, all rejected. Those are exactly the swaps the
...
EAN-13 that carries an ISSN on a magazine adds 2 variant digits that are likewise not derivable.
...
argument for a masking `toString` looks like it applies. It does not, because the 2 differ in what
...
**The TAC is 8 digits with no Final Assembly Code beside it.** Pre-2004 IMEIs were TAC (6) + FAC
...
reading is right for anything currently in circulation, `tac` is the 8-digit field and there is
no `fac` getter. `reportingBodyIdentifier` exposes the 2 leading digits, which is the one
subdivision of the TAC that has not moved.

**16 digits are named as an IMEISV, not called a miscount.** An IMEISV trades the check digit
for a 2-digit software version, so a 16-digit input is a real identifier for the same
...
## Gtin: 4 lengths, one number, padded to 14

**Every length folds to GTIN-14, and the padding cannot lie.** GS1's own guidance is to store a GTIN
of any length zero-padded to 14 digits in one field, and that is exactly what `value` holds,
...
implementation weighted from the left and so agreed with GS1 only at exactly 12 digits.

**3 getters and a `shortestForm`, because one answer does not fit.** `value` is the storage
...
the prefix to `978`/`979`, carves out the ISMN range, and rebuilds the 10-digit form. Parse as
