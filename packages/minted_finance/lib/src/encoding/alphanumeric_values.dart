...
/// The value `A` maps to, and so the floor above which a value spells 2 digits rather than one.
const letterOffset = 10;

/// The value of a single alphanumeric character: `0`-`9` map to 0-9, `A`-`Z` to 10-35.
///
/// One convention, 2 standards: ISO 13616 folds these into IBAN's mod-97, and ISO 6166 spells them
/// out before running ISIN's Luhn.
int alphanumericValue(int codeUnit) {
  final digitValue = decimalValue(codeUnit);
  if (digitValue >= 0) return digitValue;

  // Both callers gate on `[A-Z0-9]` first: the fromComponents doors take typed parts, and Isin's checksum
  // runs after its charset check.
  assert(
...
/// [input] with every letter replaced by the 2 digits spelling its [alphanumericValue], which is what
/// ISO 6166 hands to Luhn. Digits are left alone, so the result varies in length.
