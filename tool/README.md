...
naming one with nothing pending errors with the list. Releasing 2 packages means running
...
**annotated tag** with that message, and if your git config has `tag.gpgSign=true`, the
...
`### Removed` into all 6 new packages, where the honest heading was `### Added`.
...
touched, so one spanning 2 writes its title into both. Titles therefore need to read sensibly
...
and sits outside the pipeline-owned set, so it is hand-editable.
...
`git commit`, because running the dry-run against the working tree mid-execute would
...
- **Post-commit, pre-tag failure** (dry-run rejected the prep commit):
  ...
  before the run started. No remote tag is ever created in this case, since the
...
Otherwise it falls back to whatever `dart` is on `PATH`, so a non-FVM contributor can run it
