...
void main() {
  feature('GeoCoordinate', () {
    ...
    scenario('from throws MintedFormatException naming the offending part', () {
      check(() => GeoCoordinate.from(latitude: 91, longitude: 0))
          .throws<MintedFormatException>()
          .has((error) => error.failure, 'failure')
          .equals(const GeoCoordinateLatitudeOutOfRange(91));
      check(() => GeoCoordinate.from(latitude: 0, longitude: 181))
          .throws<MintedFormatException>()
    ...
    scenario('a negative zero is normalised away', () {
      ...
      final origin = GeoCoordinate.from(latitude: -0, longitude: -0);

      check(origin.iso6709).equals('+00+000/');
      check(origin.hashCode).equals(GeoCoordinate.from(latitude: 0, longitude: 0).hashCode);
    });
    ...
    scenario('a transposed pair is not the same coordinate', () {
      check(
        GeoCoordinate.from(latitude: 40, longitude: 2) ==
            GeoCoordinate.from(latitude: 2, longitude: 40),
      ).isFalse();
    });
