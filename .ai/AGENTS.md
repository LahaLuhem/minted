...
name. `luhn_check_digit.dart` is core's, finance and identifiers both running it, where
`iban_check_digits.dart` is `minted_finance`'s because only `Iban` does. That keeps core to what is
...
`tryFrom` and neither parse door. Usually a range over a number, though `Percentage` constrains the
*unit* instead and bounds nothing but finiteness, and a constraint on a `String` qualifies on the same
...
file per type, so `minted_finance/lib/src/failures/iban_failure.dart` holds `IbanFailure`. The
...
**A constant that 2 files both need goes in a helper subfolder, but only when it is the same
*concept*.** `cosmeticSeparators` was 5 identical copies of one normalisation rule, and
...
and `Issn` both have a `_groupSize` of 4, but IBAN groups repeatedly every 4 where ISSN splits
once after 4, and 2 of the check-digit moduli are both 10 for unrelated standards. Hoisting
those would couple facts that are free to move independently. Sharing a top-level constant also
means dropping its `_`, a leading underscore being library-private in Dart. Privacy then comes from
`lib/src/` and from not re-exporting it.
...
where 2 standards share one (`mod11_check_character.dart`) and for the standard where it is the
...
1. **Every value type follows the same contract:**
   - a private primary constructor (`._`), and no public constructor ever
   - `static ParseOutcome<F, T> parse(String)`, carrying the type's own failure
   - `static T? tryParse(String)`, derived from it
   - assembly factories (`fromComponents`, `from`, …) returning that same outcome
   - value equality, a canonical string form, and per-type render helpers
   - a failure vocabulary in the package's `failures/`

   One carve-out: an assembly door whose every parameter already carries its own invariants cannot
   fail, so it returns the value directly and may be a named factory (`Geohash.from`). **No door
   throws.** A door that can fail says so in its return type, and `ParseOutcome.getOrThrow` is the
   one place a caller opts into a throw, raising `MintedFormatError`. This is the package's identity,
   not a preference. Full spec:
   [`CODESTYLE.md#value-type-contract`](../CODESTYLE.md#value-type-contract). Rationale in
   [rationale][apx-per-type-failures] and [rationale][apx-claim-in-source].
2. **`minted` is the core and the lowest package, and every other package is a sibling depending
   on it.** A consumer takes `minted` plus one or more siblings. What gets extracted *into* core is
...
3. **A package's public API lives only in its barrel**, `lib/<package>.dart`, which re-exports from
   `lib/src/`. Don't make users import `package:…/src/…`. Core's cross-package internals go in its
   `lib/src/shared/`.
...
- No magic numbers in `lib/` code. A type's constants live on that type, cross-package ones under
  core's `lib/src/shared/`.
...
- **Mark recommendations with `★`.** Prefix your preferred option in every set with `★` so the
  user can scan and reply by echoing or overriding ("★ for 1-4, change 5 to B"). A later
...
- **Prefer an existing package over a custom solution.** Before hand-rolling validation, data, or a
  ...
  core's `lib/src/shared/`, not a micro-dependency, so the dependency-light core stays honest.
...
- **The user manages git state, and some tracked files won't show in `git status`.** The user may mark
